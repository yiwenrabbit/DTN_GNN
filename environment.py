...
class Env:
    def __init__(self, edge_num, vehicle_num, load_from_file=False, vehicle_file_name="vehicle_positions.json",
        ...
        self.last_update_step = self.Initial_last_update_step()
        #self.task_spread_factor = self.tasks[0].calculate_task_spread()

    def Initial_last_update_step(self):
        temp = {}
        for sub_task in self.all_subtasks:
            temp[sub_task.subtask_id] = -1
        return temp
    ...
    def dt_update_after_actions(self, edge_selection, decision_probs):
        update_ratio_all = np.zeros(len(self.all_subtasks))
        update_size = 0
        ratio = 0.0
        #finish_reward = 0
            ...
            if sub_task.subtask_done == 0:
                ...
                if sub_task.subtask_ready == 0:
                    ...
                    if sub_task.off_tag == 0:
                        ...
                        # 任务没有在卸载
                        if sample > decision:
                            print("End current task:",sub_task.subtask_id,"at step",self.current_step)
                            ...
                            update_ratio_all[index] = update_ratio

                            # 这个是达到100%DT更新的情况
                            if dt.update_done == True:
                                print("reach 100% DT update")
                                accuracy = dt.return_accuracy()
                                sub_task.compute_size_accu(accuracy)  # 根据DT的更新量决定卸载任务数据量
                                sub_task.off_tag = 0
        ...
        print("current step update ratio:",update_ratio_all)
        # 返回该时隙可以卸载的任务
        return offload_subtasks, update_size, update_ratio_all

    # def find_unique_relevant_steps(self, transitions):
    #     step_map = {}
    #
    #     for t in range(len(transitions) - 1):
    #         (_, _, _, ready_mask, _, _, off_mask,
    #          *_, new_ready_mask, _, _, new_off_mask, _, _) = transitions[t]
    #
    #         ready_mask = np.array(ready_mask)
    #         off_mask = np.array(off_mask)
    #         new_off_mask = np.array(new_off_mask)
    #
    #         # 1. DT 更新中（正在更新）：ready 且未卸载，下一步也未卸载
    #         dt_mask = (ready_mask == 1) & (off_mask == 1) & (new_off_mask == 1)
    #
    #         # 2. 开始卸载：off_mask 从 1 → 0
    #         start_mask = (off_mask == 1) & (new_off_mask == 0)
    #
    #         if not np.any(dt_mask) and not np.any(start_mask):
    #             continue  # 这个 step 没有任何事件，跳过
    #
    #         # 如果 step 第一次出现，初始化
    #         if t not in step_map:
    #             step_map[t] = {
    #                 "step": t+1,
    #                 "types": set(),  # 用 set 自动去重
    #                 "mask": np.zeros_like(ready_mask, dtype=bool)
    #             }
    #
    #         if np.any(dt_mask):
    #             step_map[t]["types"].add("dt_updating")
    #             step_map[t]["mask"] |= dt_mask  # 合并 mask
    #
    #         if np.any(start_mask):
    #             step_map[t]["types"].add("start_offload")
    #             step_map[t]["mask"] |= start_mask  # 合并 mask
    #
    #     # 输出为统一格式
    #     result = []
    #     for step_data in sorted(step_map.values(), key=lambda x: x["step"]):
    #         result.append({
    #             "step": step_data["step"],
    #             "types": list(step_data["types"]),
    #             "mask": step_data["mask"].tolist()
    #         })
    #
    #     return result
        ...
        for t in range(len(transitions) - 1):
            ...
            if t not in step_map:
                step_map[t] = {
                    "step": t + 1,
                    "types": set(),
                    "dt_updating_mask": np.zeros_like(ready_mask, dtype=bool),
                    "start_offload_mask": np.zeros_like(ready_mask, dtype=bool),
                }

            if np.any(dt_mask):
                step_map[t]["types"].add("dt_updating")
                step_map[t]["dt_updating_mask"] |= dt_mask
                record_tag = True

            if np.any(start_mask):
                step_map[t]["types"].add("start_offload")
                step_map[t]["start_offload_mask"] |= start_mask
                record_tag = True

            if record_tag:
                filted_transitions.append(transitions[t])
        # 输出为统一格式
        result = []
        for step_data in sorted(step_map.values(), key=lambda x: x["step"]):
            result.append({
                "step": step_data["step"],
                "types": list(step_data["types"]),
                "dt_updating_mask": step_data["dt_updating_mask"].tolist(),
                "start_offload_mask": step_data["start_offload_mask"].tolist()
            })

        return result, filted_transitions

    #找出DT更新的步骤
    def return_update_steps(self, relevant_steps):
        updated_steps_dict = {}

        for step_info in relevant_steps:
            step = step_info["step"]
            dt_mask = step_info.get("dt_updating_mask", [])
            #start_mask = step_info.get("start_offload_mask", [])


            # 合并两个 mask
            #combined_mask = [s or d for s, d in zip(start_mask, dt_mask)]

            for idx, selected in enumerate(dt_mask):
                if not selected:
                    continue
                subtask_id = self.tasks[0].subtasks[idx].subtask_id
                if subtask_id not in updated_steps_dict:
                    updated_steps_dict[subtask_id] = []
                updated_steps_dict[subtask_id].append(step)

        return updated_steps_dict

    #为更新的步骤生成时延惩罚
    def return_delay_penalty(self,updated_steps_dict):
        task = self.tasks[0]
        ...
        for subtask in task.subtasks:
            dep = task.get_descendant_subtasks(subtask)
            subtask_dependency[subtask.subtask_id] = dep
            all_dependencies.append(dep)

            total_dependency = sum(all_dependencies) if all_dependencies else 1.0

        # 创建惩罚分配结构
        delay_penalty_dict = {}

        for subtask_id, steps in updated_steps_dict.items():
            # 分配给该子任务的总惩罚
            dep = subtask_dependency.get(subtask_id, 0)
            total_subtask_penalty = (dep / total_dependency) * (1-task_delay/task.origin_task_delay)

            # 分配给每个step的惩罚（线性递增，越靠后的step惩罚越重）
            num_steps = len(steps)
            step_penalty = {}
            for i, step in enumerate(sorted(steps)):
                # (step索引+1)/总step数 保证后面越大
                weight = (i + 1) / num_steps
                step_penalty[step] = total_subtask_penalty * weight

            delay_penalty_dict[subtask_id] = {
                "total_penalty": total_subtask_penalty,
                "steps": step_penalty
            }
        return delay_penalty_dict

    def reward_after_finished(self, transitions):
        off_mask = []
        done_mask = []
        ready_mask = []
        new_off_mask = []
        ...
        for trans in transitions:
            done_mask.append(trans[5])
            ready_mask.append(trans[3])
            new_ready_mask.append(trans[12])
            new_done_mask.append(trans[14])
            off_mask.append(trans[6])
            new_off_mask.append(trans[15])
        print(done_mask)
        print(new_done_mask)
        print(ready_mask)
        print(new_ready_mask)
        print(new_done_mask)
        print(off_mask)
        print(new_off_mask)
    ...
    def step(self, DT_actions, new_action=False):
        ...
        # 需要做卸载决策的子任务集合
        offload_subtasks, update_size, update_ratio_all = self.dt_update_after_actions(edge_selection, completion_rates)
        ...
        return reward, x, edges, network_states, ready, edge, done_mask, off_mask, update_ratio_all
    ...
    def update_rewards(self, filted_transitions, delay_penalty_dict, accuracy_step_dict, relevant_steps, average_accuracy):
        for index, item in enumerate(relevant_steps):
            ...
            update_mask = item["dt_updating_mask"]
            finish_mask = item["start_offload_mask"]
            #current_transtions = filted_transitions[index]
            for task_index, update_tag in enumerate(update_mask):
                task_id = self.all_subtasks[task_index].subtask_id
                if update_tag:
                    filted_transitions[index][8] += -delay_penalty_dict[task_id]["steps"][current_step]
                if finish_mask[task_index]:
                    if task_id not in accuracy_step_dict:
                        print(f"[⚠️ Warning] task_id '{task_id}' not found in accuracy_step_dict. Skipping.")
                        continue
                    if accuracy_step_dict[task_id][0] < average_accuracy:
                        filted_transitions[index][8] += accuracy_step_dict[task_id][0] - average_accuracy
        return filted_transitions
